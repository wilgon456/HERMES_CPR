...
- gateway가 `draining` 상태에서 허용 시간 이상 멈춰 있을 때 → `hermes gateway restart`
...
- `stale_confirmations`: degraded stale 상태가 몇 번 연속 확인돼야 재시작할지
...
- `draining`은 grace period 이전에 재시작하지 않음
